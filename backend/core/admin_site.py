...
from foundation.models import Article, ContactSubmission, Donation, Page, PageView, SiteSettings, Visitor
...
class FoundationAdminSite(AdminSite):
    ...
    def index(self, request, extra_context=None):
        ...
        total_donations = paid_donations.aggregate(total=Sum("amount"), donors=Count("id"))
        campaign_rows = list(
        ...
        pie_colors = ["#1f5f7a", "#f59e0b", "#10b981", "#3b82f6", "#8b5cf6", "#ef4444", "#14b8a6", "#64748b"]
        running_percent = 0
        campaign_breakdown = []
        for index, row in enumerate(campaign_rows):
            raised = row["raised"] or 0
            percent = round((raised / total_campaign_raised) * 100, 1) if total_campaign_raised else 0
